    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg border border-gray-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-small font-semibold text-gray-900">Decryption Progress</span>
          <span className="text-small text-gray-600">{progressCount}/{messages.length} cracked</span>
        </div>
            ...
            >
              ...
              <div className="mb-4">
                ...
                <div className="bg-gray-100 p-3 rounded border border-gray-300">
                  <p className="font-mono text-body text-orange-600 break-all">{message.encryptedText}</p>
                </div>
              </div>
              ...
              <div className="mb-4">
                <label className="block text-small font-medium text-gray-900 mb-2">
                  Shift Value: <span className={isCorrect ? "text-green-600" : "text-blue-600"}>{shift}</span>
              ...
              {message.hint && (
                <div>
                  <button
                    ...
                    className="flex items-center gap-2 text-small text-blue-600 hover:text-blue-500 transition-colors"
                  >
                  ...
                  {showHints[message.id] && (
                    <div className="mt-2 bg-warning-900/20 border border-warning-700 p-3 rounded">
                      <p className="text-small text-gray-900">{message.hint}</p>
                    </div>
                  )}
                </div>
              )}
      ...
      {allCorrect && (
        <div className="bg-success-900/20 border border-success-700 p-4 rounded-lg animate-in fade-in duration-300">
          <p className="text-body text-green-600 font-semibold">
      ...
      <button
        ...
        className={`w-full px-6 py-4 text-white font-semibold rounded-lg transition-all duration-200 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${
          allCorrect ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
        }`}
      >
        <CheckCircle className="w-5 h-5" />
